...
method flat(self: Pattern, types: varargs[string]):
    seq[Pattern] {.base, gcsafe.} = assert false

method match(self: Pattern, left: seq[Pattern], collected: seq[Pattern] = @[]):
    MatchResult {.base, gcsafe.} = assert false
...
method single_match(self: ChildPattern, left: seq[Pattern]): SingleMatchResult {.base, gcsafe.} =
    assert false

method match(self: ChildPattern, left: seq[Pattern], collected: seq[Pattern] = @[]): MatchResult =
...
method str(self: Option): string =
    "Option($#, $#, $#, $#)".format(self.short.str, self.long.str, self.argcount, self.value.str)

method match(self: Required, left: seq[Pattern], collected: seq[Pattern] = @[]): MatchResult =
...
method match(self: Optional, left: seq[Pattern], collected: seq[Pattern] = @[]): MatchResult =
    result = (true, left, collected)
    for pattern in self.children:
        result = pattern.match(result.left, result.collected)
    result.matched = true

method match(self: OneOrMore, left: seq[Pattern], collected: seq[Pattern] = @[]): MatchResult =
...
method match(self: Either, left: seq[Pattern], collected: seq[Pattern] = @[]): MatchResult =
    var found = false
    for pattern in self.children:
...
## else:
##     argv ::= [ long | shorts | argument ]* [ '--' [ argument ]* ] ;
proc parse_argv(tokens: TokenStream, options: var seq[Option], options_first = false):
    seq[Pattern] =
    result = @[]
    while tokens.current != "":
        if tokens.current == "--":
...
# Parse printable Usage from given docstring block
